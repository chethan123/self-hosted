#!/bin/sh
# Dump service's whole program (docs/specs/backup-sidecar.md §8, "Dump service contract"): prune, check
...
# 0640 files in a 0750 directory: a dump is the whole database in plaintext, readable to the
# account that owns the directory because that is the backup sidecar's account (ADR-0006).
umask 027
...
# The archive name prefix — the app slug. Portfolio's original hardcodes its own.
...
validate() {
  ...
  # Only dumps the bundled `db` service — a package on an external Postgres deletes `db` and
...
is_ours() {
  case "$1" in
    "$DUMP_PREFIX"-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]T[0-9][0-9][0-9][0-9][0-9][0-9]Z.dump) return 0 ;;
    *) return 1 ;;
  esac
}
...
prune() {
  ...
  newest=""
  for f in "$dir"/"$DUMP_PREFIX"-*.dump; do
    [ -e "$f" ] || continue
    b=$(basename "$f")
    is_ours "$b" || continue
...
run_once() {
  ...
  s=$(stamp)
  part="$DUMP_DIR/.$DUMP_PREFIX-$s.dump.part"
  final="$DUMP_DIR/$DUMP_PREFIX-$s.dump"
  started=$(now)
...
loop() {
  rm -f "$DUMP_DIR"/."$DUMP_PREFIX"-*.dump.part
...
healthcheck() {
  newest=""
  for f in "$DUMP_DIR"/"$DUMP_PREFIX"-*.dump; do
    [ -e "$f" ] || continue
    b=$(basename "$f")
    is_ours "$b" || continue
    if [ -z "$newest" ] || [ "$b" \> "$newest" ]; then newest="$b"; fi
